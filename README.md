...
NoteFlow Atelier is a local-first, static multi-page workspace with notes, tasks, timeline planning, homework tracking, and optional Google Drive backup.
...
2. Use the launch buttons to open:
   ...
   - `Homework.html` (standalone homework organizer)
...
| `Homework.html` | Standalone homework organizer | `HomePage.html` button |
...
- **Multi-view workspace tabs** - `Today`, `Timeline`, `Notes`, `Settings`, `Homework`. Where: top nav tabs. How: click tab buttons. Storage: current view state in runtime; persisted data is still global workspace data.
...
- **Built-in AI assistant (Flow)** - Chat panel with Groq API key input, markdown-ish response rendering, copy/insert actions, fullscreen, and info panel. Where: floating mascot/chat panel. How: save Groq key, send prompt. Storage: `groq_api_key`, `chat_history`, optional `chat_model` in `localStorage`.
...
- **Workspace export JSON** - Exports pages, tasks, task order, time blocks, streaks, settings, UI metadata, schema version, timestamp, and theme info. Where: save bar/settings export buttons. How: click Export. Storage: downloaded file only.
...
### `Homework.html` + `homework.js` (Standalone Homework Organizer)

- **Simple task entry form** - Subject, task, due date, priority, and Add button. Where: standalone Homework page. How: fill fields and click Add or press Enter in task field. Storage: `hwCourses:v2` and `hwTasks:v2`.
- **Task table view** - Shows subject/task/due/priority and supports done/undo + delete row actions. Where: standalone Homework page table. How: use row action buttons. Storage: same homework localStorage keys.
- **Back navigation** - Returns to landing page. Where: top link. How: click `Back`. Storage: none.
- **Backward compatibility load path** - If v2 homework keys are empty, reads legacy keys (`homeworkCourses:v1`, `homeworkTasks:v1`). Where: standalone and embedded homework logic. How: automatic. Storage: reads legacy keys, writes v2 on next save.
...
- **Classic AI assistant panel** - Groq key-based assistant with insert/copy actions. Where: classic chat panel. How: same as Atelier flow. Storage: `localStorage` chat keys.
...
- **Record key:** `root`
- **Schema field:** `schemaVersion` (currently `3` in code)
...
- `ui` (`favoritePageId`, `defaultPageId`)
- `timeBlocks`
...
- `groq_api_key` (assistant API key)
- `chat_history` (assistant message history)
...
- **Standalone Homework page does not show expected old records**
...
## Observed but Unconfirmed (Code Paths Present, Not Wired in Current Markup)

These code paths exist in `app.js`, but matching DOM hooks are absent in current `NoteflowAtelier.html`:

- `templateModal`-based template chooser (`showTemplateModal`, `createFromTemplate`) - no `id="templateModal"` element in current page.
- Legacy progress containers (`weekly-chart`, `streak-stats`, `category-stats`, `history-list`) - render code exists but current UI uses different widgets.
- Linked-task inline input controls (`linkedTaskList`, `linkedTaskInput`, `linkedTaskAddBtn`, `addLinkedTaskBtn`) - handlers exist; controls are removed from markup.
- `useFreezeBtn` action hook - button ID is not currently present in the page.

How to verify: add corresponding DOM elements or restore previous markup and re-test those handlers.
