...
**`.sutra`** is now the default export format. Files are named `sutra_workspace_<YYYY-MM-DD>.sutra` and are ZIP packages containing `manifest.json`, `workspace.json`, an `assets/` folder, and a `metadata/` folder. The manifest identifies Sutra:
...
**Legacy `.atelier` backups still import.** The import validator accepts **both** the new `sutra-workspace` manifest and the legacy `noteflow_atelier_project` manifest, and the import dispatcher routes both `.sutra` and `.atelier` files to the same package importer. **Old backups are never broken.** You can keep restoring any `.atelier` file you exported under the old name.

**Never exported, in either format:** API keys, provider credentials, and tokens (these live in sessionStorage only). The assistant activity log is not a secret and does travel in backups.
...
> Always keep a recent backup before any major change. There is no cloud copy — your exported file is the safety net.
