...
// Google API Configuration
        ...
        function getDefaultCollegeAppWorkspace() {
            ...
            const criterionSafety = createDecisionCriterionRow({ name: 'Campus safety', weight: 3 });
            const criterionAlumni = createDecisionCriterionRow({ name: 'Alumni network', weight: 3 });
            const criterionInternships = createDecisionCriterionRow({ name: 'Internship / co-op access', weight: 4 });
            const criterionHousing = createDecisionCriterionRow({ name: 'Housing quality', weight: 2 });
            const criterionWeather = createDecisionCriterionRow({ name: 'Weather / climate', weight: 1 });
            const criterionStudyAbroad = createDecisionCriterionRow({ name: 'Study abroad programs', weight: 2 });
            const criterionGradSchool = createDecisionCriterionRow({ name: 'Grad school placement', weight: 3 });
            const criterionExtracurriculars = createDecisionCriterionRow({ name: 'Clubs & extracurriculars', weight: 2 });
            const northwood = createDecisionCollegeRow({
                ...
                scores: {
                    ...
                    [criterionSafety.id]: 7,
                    [criterionAlumni.id]: 9,
                    [criterionInternships.id]: 8,
                    [criterionHousing.id]: 6,
                    [criterionWeather.id]: 5,
                    [criterionStudyAbroad.id]: 7,
                    [criterionGradSchool.id]: 9,
                    [criterionExtracurriculars.id]: 6
                }
            });
            const lakeview = createDecisionCollegeRow({
                ...
                scores: {
                    ...
                    [criterionSafety.id]: 8,
                    [criterionAlumni.id]: 6,
                    [criterionInternships.id]: 6,
                    [criterionHousing.id]: 8,
                    [criterionWeather.id]: 7,
                    [criterionStudyAbroad.id]: 5,
                    [criterionGradSchool.id]: 6,
        ...
        function normalizeCollegeAppWorkspace(data) {
            ...
            // Merge in any new default criteria that don't already exist (matched by name)
            if (Array.isArray(sourceMatrix.criteria)) {
                const existingNames = new Set(normalized.decisionMatrix.criteria.map(c => (c.name || '').trim().toLowerCase()));
                defaults.decisionMatrix.criteria.forEach(defaultCriterion => {
                    const key = (defaultCriterion.name || '').trim().toLowerCase();
                    if (key && !existingNames.has(key)) {
                        normalized.decisionMatrix.criteria.push(createDecisionCriterionRow(defaultCriterion));
        ...
        function normalizeHexColor(value, fallback) {
            ...
            return /^#[0-9a-fA-F]{6}$/.test(raw) ? raw.toLowerCase() : String(fallback || '#000000').toLowerCase();
        }
        ...
        function isCustomThemeKey(themeName) {
            ...
            return raw.startsWith(CUSTOM_THEME_KEY_PREFIX) && raw.length > CUSTOM_THEME_KEY_PREFIX.length;
        }

        function toCustomThemeKey(themeId) {
            const normalized = String(themeId || '').trim().toLowerCase();
            return normalized ? `${CUSTOM_THEME_KEY_PREFIX}${normalized}` : '';
        }

        function getCustomThemeIdFromKey(themeName) {
            if (!isCustomThemeKey(themeName)) return '';
            return String(themeName).trim().toLowerCase().slice(CUSTOM_THEME_KEY_PREFIX.length);
        }

        function normalizeCustomThemeRecord(rawTheme, index = 0) {
            const source = rawTheme && typeof rawTheme === 'object' ? rawTheme : {};
            const fallbackName = `${DEFAULT_CUSTOM_THEME.name} ${index + 1}`;
            const idSeed = String(source.id || '').trim().toLowerCase().replace(/[^a-z0-9_-]/g, '');
            return {
                id: idSeed || `ct_${generateId().replace(/[^a-z0-9_-]/gi, '').toLowerCase()}`,
        ...
        function ensureCustomThemeSettingsState() {
            if (!appSettings) return;
            const incoming = Array.isArray(appSettings.customThemes) ? appSettings.customThemes : [];
            const seen = new Set();
            const normalizedThemes = [];
            incoming.forEach((themeEntry, index) => {
                const normalized = normalizeCustomThemeRecord(themeEntry, index);
            ...
            appSettings.customThemes = normalizedThemes;

            const activeId = String(appSettings.activeCustomThemeId || '').trim().toLowerCase();
            const isActiveValid = normalizedThemes.some(themeEntry => themeEntry.id === activeId);
            appSettings.activeCustomThemeId = isActiveValid
                ? activeId
                : (normalizedThemes[0] ? normalizedThemes[0].id : '');
        }

        function getCustomThemes() {
            ensureCustomThemeSettingsState();
            return appSettings && Array.isArray(appSettings.customThemes) ? appSettings.customThemes : [];
        }

        function getCustomThemeById(themeId) {
            const normalizedId = String(themeId || '').trim().toLowerCase();
            if (!normalizedId) return null;
            return getCustomThemes().find(themeEntry => themeEntry.id === normalizedId) || null;
        }
        ...
        function createCustomTheme(name, colors = {}) {
            if (!appSettings) return null;
            ensureCustomThemeSettingsState();
            const nextTheme = normalizeCustomThemeRecord({
                id: `ct_${generateId().replace(/[^a-z0-9_-]/gi, '').toLowerCase()}`,
                name: name || DEFAULT_CUSTOM_THEME.name,
                ...colors
            }, getCustomThemes().length);
            appSettings.customThemes.push(nextTheme);
        ...
        function normalizeStoredThemeKey(themeName, fallback = DEFAULT_THEME_KEY, allowCustom = false) {
            ...
            if (allowCustom && normalized === 'custom') return 'custom';
            if (allowCustom && isCustomThemeKey(normalized)) return normalized;
            if (themes[normalized]) return normalized;
            const fallbackNormalized = normalizeThemeName(fallback);
            if (allowCustom && isCustomThemeKey(fallbackNormalized)) return fallbackNormalized;
            return themes[fallbackNormalized] ? fallbackNormalized : DEFAULT_THEME_KEY;
        }

        function getThemeMode(themeName) {
            if (isCustomThemeKey(themeName)) {
                const themeEntry = getCustomThemeById(getCustomThemeIdFromKey(themeName));
                return themeEntry && isDarkHexColor(themeEntry.bgPrimary) ? 'dark' : 'light';
            }
            const normalized = normalizeStoredThemeKey(themeName, DEFAULT_THEME_KEY);
            return themes[normalized] && themes[normalized].mode === 'dark' ? 'dark' : 'light';
        }

        function getCurrentPageThemeKey() {
            const currentPage = pages.find(p => p.id === currentPageId);
            if (currentPage && currentPage.theme) {
                return normalizeStoredThemeKey(currentPage.theme, globalTheme || DEFAULT_THEME_KEY, true);
            }
            return normalizeStoredThemeKey(globalTheme, DEFAULT_THEME_KEY, true);
        }
    ...
    const MAX_COMMITS_PER_DAY = Infinity;
        ...
        function renderCollegeAppDecisionMatrix() {
            ...
            /* ------ Criteria panel (compact two-column grid) ------ */
            ...
            /* ------ College Scores header (kept for backward-compat) ------ */
            if (collegesHeader) {
            ...
            if (collegesBody) {
                if (!colleges.length) {
                    collegesBody.innerHTML = `<div class="dm-empty"><i class="fas fa-university"></i>No colleges in matrix yet. Add at least one college to compare.</div>`;
                } else {
                    collegesBody.innerHTML = colleges.map(college => {
                        ...
                        const criterionRows = criteria.map(criterion => {
                            const scoreVal = Math.max(0, Math.min(10, normalizeFiniteNumber(college.scores && college.scores[criterion.id], 0)));
                            ...
                            <div class="dm-card-row${isZero ? ' dm-card-row-zero' : ''}">
                                ...
                                <input type="number" min="0" max="10" step="0.1" class="dm-score-input" data-collegeapp-row-id="${escapeHtml(String(college.id))}" data-collegeapp-score-criterion="${escapeHtml(String(criterion.id))}" value="${escapeHtml(String(scoreVal))}">
                            </div>`;
                        }).join('');
                        return `
                        <div class="dm-college-card">
                            <div class="dm-college-card-head">
                                <input class="dm-college-name-input" data-collegeapp-collection="decisionColleges" data-collegeapp-row-id="${escapeHtml(String(college.id))}" data-collegeapp-field="name" value="${escapeHtml(String(college.name || ''))}" placeholder="College name">
                                <div class="dm-college-card-score">
                                    <span class="dm-weighted-total">${normScore.toFixed(2)}</span>
                                    <span class="dm-weighted-sub">/ 10</span>
                                </div>
                                <button type="button" class="icon-btn collegeapp-delete-row-btn dm-college-card-delete" data-collegeapp-collection="decisionColleges" data-collegeapp-row-id="${escapeHtml(String(college.id))}" aria-label="Delete college"><i class="fas fa-trash"></i></button>
                            </div>
        ...
        function initCollegeAppWorkspaceUI() {
            ...
            root.addEventListener('click', (event) => {
                ...
                // Criteria panel collapse toggle
                const criteriaToggle = event.target.closest('#dmCriteriaToggle');
                if (criteriaToggle) {
                    const criteriaPanel = document.querySelector('.dm-criteria-panel');
                    if (criteriaPanel) {
                        criteriaPanel.classList.toggle('collapsed');
                        const icon = criteriaToggle.querySelector('i');
                        if (icon) {
                            icon.className = criteriaPanel.classList.contains('collapsed') ? 'fas fa-chevron-right' : 'fas fa-chevron-down';
                        }
                    }
                    return;
                }
        ...
        function normalizeHabitTrackerHabitsCollection(rawHabits) {
            const seenIds = new Set();
            const fallbackCreatedAt = new Date().toISOString();
            return (Array.isArray(rawHabits) ? rawHabits : []).reduce((acc, rawHabit) => {
                if (!rawHabit || typeof rawHabit !== 'object') return acc;
                const habit = rawHabit;
                let id = String(habit.id || '').trim();
                if (!id || seenIds.has(id)) id = generateId();
                seenIds.add(id);
                acc.push({
                    ...habit,
        ...
        function normalizeHabitTrackerDayStateMap(rawDayStates) {
            const normalized = {};
            if (!rawDayStates || typeof rawDayStates !== 'object') return normalized;
            Object.entries(rawDayStates).forEach(([dayKey, rawState]) => {
                if (!dayKey) return;
                const idsRaw = Array.isArray(rawState)
                    ? rawState
                    : (rawState && Array.isArray(rawState.completedHabitIds) ? rawState.completedHabitIds : []);
                const uniqueIds = Array.from(new Set(idsRaw.map(id => String(id || '').trim()).filter(Boolean)));
                if (!uniqueIds.length) return;
                normalized[dayKey] = { completedHabitIds: uniqueIds };
            });
            return normalized;
        }

        function normalizeLifeHabitCompletionMap(rawCompletions) {
            const normalized = {};
            if (!rawCompletions || typeof rawCompletions !== 'object') return normalized;
            Object.entries(rawCompletions).forEach(([dayKey, ids]) => {
                if (!dayKey || !Array.isArray(ids)) return;
                const uniqueIds = Array.from(new Set(ids.map(id => String(id || '').trim()).filter(Boolean)));
                if (!uniqueIds.length) return;
                normalized[dayKey] = uniqueIds;
            });
            return normalized;
        }

        function syncHabitTrackersAcrossViews() {
            if (!lifeWorkspace || typeof lifeWorkspace !== 'object') {
                lifeWorkspace = getDefaultLifeWorkspace();
            }

            const lifeHabits = Array.isArray(lifeWorkspace.habits)
                ? lifeWorkspace.habits.map(row => createLifeHabitRow(row))
                : [];
            const todayHabits = normalizeHabitTrackerHabitsCollection(habits);
            const todayHabitsById = new Map(todayHabits.map(habit => [String(habit.id), habit]));
            const lifeHabitsById = new Map();

            lifeHabits.forEach(row => {
                lifeHabitsById.set(String(row.id), row);
            });

            todayHabits.forEach(habit => {
                const id = String(habit.id);
                if (!lifeHabitsById.has(id)) {
                    lifeHabitsById.set(id, createLifeHabitRow({
                        id,
                        name: habit.name || '',
                ...
                const lifeRow = lifeHabitsById.get(id);
                if (!String(lifeRow.name || '').trim() && String(habit.name || '').trim()) {
                    lifeRow.name = String(habit.name || '').trim();
                }
            });

            const mergedLifeHabits = [];
            const mergedLifeHabitIds = new Set();
            lifeHabits.forEach(row => {
                const id = String(row.id);
                if (mergedLifeHabitIds.has(id)) return;
                const merged = lifeHabitsById.get(id);
                if (!merged) return;
                mergedLifeHabits.push(createLifeHabitRow(merged));
                mergedLifeHabitIds.add(id);
            });
            todayHabits.forEach(habit => {
                const id = String(habit.id);
                if (mergedLifeHabitIds.has(id)) return;
                const merged = lifeHabitsById.get(id) || createLifeHabitRow({
                    id,
                    name: habit.name || '',
                    category: habit.category || '',
                    targetPerWeek: habit.targetPerWeek
            ...
            const validHabitIds = new Set(mergedLifeHabits.map(row => String(row.id)));
            const lifeCompletionMap = normalizeLifeHabitCompletionMap(lifeWorkspace.habitCompletions);
            const todayCompletionMap = normalizeHabitTrackerDayStateMap(habitDayStates);
            const mergedCompletionMap = {};
            const completionKeys = new Set([
                ...Object.keys(lifeCompletionMap),
                ...Object.keys(todayCompletionMap)
            ]);

            completionKeys.forEach(dayKey => {
                const mergedSet = new Set();
                (lifeCompletionMap[dayKey] || []).forEach(id => {
                    const key = String(id || '').trim();
                    if (validHabitIds.has(key)) mergedSet.add(key);
                });
                const todayIds = (todayCompletionMap[dayKey] && todayCompletionMap[dayKey].completedHabitIds) || [];
                todayIds.forEach(id => {
                    const key = String(id || '').trim();
                    if (validHabitIds.has(key)) mergedSet.add(key);
                });
                if (!mergedSet.size) return;
                mergedCompletionMap[dayKey] = Array.from(mergedSet);
            });

            const createdAtFallback = new Date().toISOString();
            const mergedTodayHabits = mergedLifeHabits.map(row => {
                const existing = todayHabitsById.get(String(row.id));
                return {
                    ...(existing || {}),
                    id: String(row.id),
                    name: typeof row.name === 'string' ? row.name : String(row.name || ''),
                    category: typeof row.category === 'string' ? row.category : '',
                    targetPerWeek: Math.max(1, Math.min(14, Math.floor(normalizeFiniteNumber(row.targetPerWeek, 7)))),
                    isActive: existing ? existing.isActive !== false : true,
                    createdAt: existing && typeof existing.createdAt === 'string' ? existing.createdAt : createdAtFallback
                };
            });

            const mergedDayStates = {};
            Object.entries(mergedCompletionMap).forEach(([dayKey, ids]) => {
                const filteredIds = Array.from(new Set(
                    (Array.isArray(ids) ? ids : [])
                        .map(id => String(id || '').trim())
                        .filter(id => id && validHabitIds.has(id))
                ));
                if (!filteredIds.length) return;
                mergedDayStates[dayKey] = { completedHabitIds: filteredIds };
            });

            lifeWorkspace.habits = mergedLifeHabits;
            lifeWorkspace.habitCompletions = mergedCompletionMap;
            habits = mergedTodayHabits;
        ...
        function initWorkspaceUI() {
            ...
            if (customThemeSelect && customThemeSelect.dataset.bound !== 'true') {
        ...
        function applyThemeToSelection(applyToPage) {
            ...
            if (themeApplyMode === 'all') {
                ...
                shouldApplyToCurrentPage = true;
            } else if (themeApplyMode === 'current') {
                applyToPage(pages.find(p => p.id === currentPageId));
                shouldApplyToCurrentPage = true;
            } else if (themeApplyMode === 'custom') {
                selectedPagesForTheme.forEach(pageId => {
                    applyToPage(pages.find(p => p.id === pageId));
                });
                shouldApplyToCurrentPage = selectedPagesForTheme.includes(currentPageId);
            }
            return shouldApplyToCurrentPage;
        }
        ...
        function loadCustomThemeIntoColorInputs(themeEntry) {
            if (!themeEntry) return;
            const normalized = normalizeCustomThemeRecord(themeEntry, 0);
            const bgPrimaryEl = document.getElementById('customBgPrimary');
        ...
        function getCustomThemeLabel(themeKey) {
            if (!isCustomThemeKey(themeKey)) return 'Custom';
            const themeEntry = getCustomThemeById(getCustomThemeIdFromKey(themeKey));
            return themeEntry ? themeEntry.name : 'Custom';
        }

        function refreshCustomThemeControls() {
            ensureCustomThemeSettingsState();
            ...
            if (selectEl) {
                ...
                selectEl.innerHTML = '';
                themesList.forEach((themeEntry, index) => {
                    const option = document.createElement('option');
                    option.value = themeEntry.id;
                    option.textContent = `${index + 1}. ${themeEntry.name}`;
                    selectEl.appendChild(option);
                });
                const activeId = appSettings ? appSettings.activeCustomThemeId : '';
                const nextValue = themesList.some(themeEntry => themeEntry.id === activeId)
                    ? activeId
                    : (themesList.some(themeEntry => themeEntry.id === previousValue) ? previousValue : (themesList[0]?.id || ''));
                if (nextValue) selectEl.value = nextValue;
                if (appSettings) appSettings.activeCustomThemeId = nextValue || '';
                selectEl.disabled = themesList.length === 0;
            }
            ...
            if (activeTheme) {
                loadCustomThemeIntoColorInputs(activeTheme);
            } else {
        ...
        function showCustomPromptDialog(options = {}) {
            const opts = {
                title: 'Enter Value',
                label: 'Value',
                defaultValue: '',
            ...
            if (!modal || !titleEl || !labelEl || !inputEl || !cancelBtn || !confirmBtn) {
                console.warn('Custom prompt modal is unavailable');
                return Promise.resolve(null);
            }

            if (typeof activeCustomPromptResolver === 'function') {
                const previousResolver = activeCustomPromptResolver;
                activeCustomPromptResolver = null;
            ...
            return new Promise((resolve) => {
                ...
                const closePrompt = (result) => {
                    ...
                    if (activeCustomPromptResolver === closePrompt) {
                        activeCustomPromptResolver = null;
                    }
                    resolve(result);
                };
                ...
                const onConfirm = () => closePrompt(String(inputEl.value || ''));
                ...
                activeCustomPromptResolver = closePrompt;
        ...
        function showCustomThemeSetupDialog(options = {}) {
            const opts = {
                title: 'Create Custom Theme',
                subtitle: 'Pick a name and starter colors.',
                nameLabel: 'Theme Name',
                defaultName: DEFAULT_CUSTOM_THEME.name,
            ...
            const modal = document.getElementById('customThemeSetupModal');
            const titleEl = document.getElementById('customThemeSetupTitle');
            const subtitleEl = document.getElementById('customThemeSetupSubtitle');
            const nameLabelEl = document.getElementById('customThemeSetupNameLabel');
            const nameInput = document.getElementById('customThemeSetupNameInput');
            const bgPrimaryInput = document.getElementById('customThemeSetupBgPrimary');
            ...
            return new Promise((resolve) => {
                ...
                const updatePreview = () => {
                    ...
                    previewCard.style.borderColor = `rgba(${accentR}, ${accentG}, ${accentB}, 0.45)`;
                    previewName.textContent = previewLabel;

                    previewAccentChip.style.background = `rgba(${accentR}, ${accentG}, ${accentB}, 0.2)`;
                    previewAccentChip.style.borderColor = `rgba(${accentR}, ${accentG}, ${accentB}, 0.55)`;
                    previewAccentChip.style.color = textPrimary;
                ...
                const closeSetup = (result) => {
                    ...
                    bgPrimaryInput.removeEventListener('input', updatePreview);
                    ...
                    if (activeCustomThemeSetupResolver === closeSetup) {
                        activeCustomThemeSetupResolver = null;
                    }
                    resolve(result);
                };
                ...
                const onConfirm = () => {
                    ...
                    const payload = {
                        name: String(nameInput.value || '').trim() || fallbackName,
                        colors: {
                ...
                bgPrimaryInput.addEventListener('input', updatePreview);
        ...
        async function ensureFirstCustomThemeSetup() {
            ensureCustomThemeSettingsState();
            if (getCustomThemes().length > 0) return true;
            const fallbackName = `${DEFAULT_CUSTOM_THEME.name} 1`;
            const setupResult = await showCustomThemeSetupDialog({
                title: 'Create First Custom Theme',
                subtitle: 'Set your first custom theme name and colors.',
                nameLabel: 'Theme Name',
                defaultName: DEFAULT_CUSTOM_THEME.name,
                fallbackName,
                confirmText: 'Create Theme',
        ...
        function applyCustomThemeById(themeId, options = {}) {
            const opts = {
                showToast: true,
                ...options
            };
            ...
            if (appSettings) appSettings.activeCustomThemeId = customTheme.id;
            const themeKey = toCustomThemeKey(customTheme.id);
            syncPresetCardsWithTheme(themeKey);
            loadCustomThemeIntoColorInputs(customTheme);
            ...
            saveThemeSettings();
            renderPagesList();
            if (opts.showToast) showToast(`Theme applied: ${customTheme.name}`);
            refreshCustomThemeControls();
            syncSettingsControls();
            return true;
        }
        ...
        async function applyPresetTheme(themeName) {
            const rawTheme = String(themeName || '').trim().toLowerCase();
            if (rawTheme === 'custom-theme') {
                if (!await ensureFirstCustomThemeSetup()) return;
                const activeTheme = getActiveCustomTheme();
                if (!activeTheme) {
                    showToast('Create a custom theme first');
                    return;
                }
                applyCustomThemeById(activeTheme.id);
                return;
            }
            ...
            saveThemeSettings();
            renderPagesList();
            const themeLabel = themes[normalizedTheme] ? themes[normalizedTheme].name : 'Custom';
        ...
        async function addCustomTheme() {
            if (!appSettings) return;
            ensureCustomThemeSettingsState();
            const nextCount = getCustomThemes().length + 1;
            const suggestedName = `${DEFAULT_CUSTOM_THEME.name} ${nextCount}`;
            const setupResult = await showCustomThemeSetupDialog({
                title: 'Add Custom Theme',
                subtitle: 'Create another custom theme with its own color palette.',
                nameLabel: 'Theme Name',
        ...
        async function applySelectedCustomTheme() {
            if (!await ensureFirstCustomThemeSetup()) return;
            const selectEl = document.getElementById('customThemeSelect');
            const selectedId = String(selectEl && selectEl.value ? selectEl.value : appSettings.activeCustomThemeId || '').trim().toLowerCase();
            if (!selectedId) {
                showToast('Select a custom theme first');
                return;
            }
            if (appSettings) appSettings.activeCustomThemeId = selectedId;
            applyCustomThemeById(selectedId);
        }

        function handleCustomThemeSelectChange() {
            const selectEl = document.getElementById('customThemeSelect');
            if (!selectEl || !appSettings) return;
            const selectedId = String(selectEl.value || '').trim().toLowerCase();
            appSettings.activeCustomThemeId = selectedId;
            persistAppData();
            const activeTheme = getCustomThemeById(selectedId);
            if (activeTheme) {
                loadCustomThemeIntoColorInputs(activeTheme);
            }
            const currentTheme = getCurrentPageThemeKey();
            if (isCustomThemeKey(currentTheme) || currentTheme === 'custom') {
                applyCustomThemeById(selectedId, { showToast: false });
            } else {
                refreshCustomThemeControls();
            }
        }
        ...
        async function applyCustomColors() {
            ...
            const activeTheme = getActiveCustomTheme();
            if (!activeTheme) return;
            const nextColors = readCustomThemeColorsFromInputs();
            activeTheme.bgPrimary = nextColors.bgPrimary;
        ...
        function loadPageTheme(pageId) {
            ...
            const rawTheme = page && page.theme ? String(page.theme).trim().toLowerCase() : String(globalTheme || '').trim().toLowerCase();
            const themeToApply = normalizeStoredThemeKey(rawTheme, globalTheme || DEFAULT_THEME_KEY, true);
            
            if (isCustomThemeKey(themeToApply)) {
                const customTheme = getCustomThemeById(getCustomThemeIdFromKey(themeToApply));
                if (customTheme) {
                    applyCustomThemeVariables(customTheme);
                    document.body.removeAttribute('data-theme');
                } else {
        ...
        function saveThemeSettings() {
            if (!appSettings) return;
            ensureCustomThemeSettingsState();
            appSettings.theme = normalizeStoredThemeKey(globalTheme, DEFAULT_THEME_KEY, true);
            appSettings.selectedPagesForTheme = selectedPagesForTheme;
            appSettings.themeApplyMode = themeApplyMode;
            appSettings.customThemes = getCustomThemes();
            appSettings.activeCustomThemeId = appSettings.activeCustomThemeId || '';
            persistAppData();
        }

        function loadThemeSettings() {
            ensureCustomThemeSettingsState();
            globalTheme = normalizeStoredThemeKey(appSettings ? appSettings.theme : DEFAULT_THEME_KEY, DEFAULT_THEME_KEY, true);
            if (appSettings) appSettings.theme = globalTheme;
            selectedPagesForTheme = appSettings && Array.isArray(appSettings.selectedPagesForTheme)
                ? appSettings.selectedPagesForTheme
                : [];
            themeApplyMode = appSettings && ['current', 'all', 'custom'].includes(appSettings.themeApplyMode)
                ? appSettings.themeApplyMode
                : 'current';
            refreshCustomThemeControls();
        ...
        function renderPagesList() {
            ...
            function renderTree(pageId, depth, parentId) {
                ...
                if (page.theme && page.theme !== 'default') {
                    const rawTheme = String(page.theme || '').trim().toLowerCase();
                    if (isCustomThemeKey(rawTheme)) {
                        const customTheme = getCustomThemeById(getCustomThemeIdFromKey(rawTheme));
                        if (customTheme) {
                            themeColor = customTheme.accent;
                            themeIndicatorTitle = `Theme: ${customTheme.name}`;
                        }
                ...
                if (page.theme && page.theme !== 'default') {
                    ...
                    themeIndicator.title = themeIndicatorTitle;
...
${String(bodyHtml || '<p>(No content)</p>')}
        ...
        function importWorkspacePayload(data) {
            ...
            if (data.globalTheme && !importedSettings) {
                const importedTheme = normalizeStoredThemeKey(data.globalTheme, DEFAULT_THEME_KEY, true);
                globalTheme = importedTheme;
                appSettings.theme = importedTheme;
            }
        ...
        const emojiCategories = {
            'Common': ['\u{1F4C4}', '\u{1F4DD}', '\u{1F4CB}', '\u{1F4DA}', '\u{1F4D6}', '\u{1F4D3}', '\u{1F4D2}', '\u{1F4D4}', '\u{1F4D5}', '\u{1F4D7}', '\u{1F4D8}', '\u{1F4D9}', '\u{1F5C2}\uFE0F', '\u{1F4C1}', '\u{1F4C2}', '\u{1F5C3}\uFE0F', '\u{1F4CC}', '\u{1F4CD}', '\u2702\uFE0F', '\u{1F4CE}', '\u{1F9F7}', '\u{1F4CF}', '\u{1F4D0}', '\u{1F58A}\uFE0F', '\u270F\uFE0F', '\u{1F58B}\uFE0F', '\u{1F58C}\uFE0F', '\u{1F9E0}', '\u{1F4A1}', '\u2705', '\u274C', '\u26A0', '\u2B50', '\u{1F31F}', '\u{1F525}', '\u{1F680}', '\u23F0', '\u231A', '\u{1F5D3}', '\u{1F4C5}', '\u{1F9ED}', '\u{1F3AF}', '\u{1F510}', '\u{1F513}', '\u{1F511}', '\u{1F514}', '\u{1F515}', '\u{1F4AC}', '\u{1F4AD}', '\u{1F4E3}', '\u{1F4E2}', '\u{1F4CA}', '\u{1F4C8}', '\u{1F4C9}', '\u{1F4BC}', '\u{1F3E0}', '\u{1F3EB}', '\u{1F30D}', '\u{1F9FE}', '\u{1F9E9}'],
            'Faces': ['\u{1F600}', '\u{1F601}', '\u{1F602}', '\u{1F603}', '\u{1F604}', '\u{1F605}', '\u{1F606}', '\u{1F607}', '\u{1F608}', '\u{1F609}', '\u{1F60A}', '\u{1F60B}', '\u{1F60C}', '\u{1F60D}', '\u{1F60E}', '\u{1F60F}', '\u{1F610}', '\u{1F611}', '\u{1F612}', '\u{1F613}', '\u{1F614}', '\u{1F615}', '\u{1F616}', '\u{1F617}', '\u{1F618}', '\u{1F619}', '\u{1F61A}', '\u{1F61B}', '\u{1F61C}', '\u{1F61D}', '\u{1F61E}', '\u{1F61F}', '\u{1F620}', '\u{1F621}', '\u{1F622}', '\u{1F623}', '\u{1F624}', '\u{1F625}', '\u{1F626}', '\u{1F627}', '\u{1F628}', '\u{1F629}', '\u{1F62A}', '\u{1F62B}', '\u{1F62C}', '\u{1F62D}', '\u{1F62E}', '\u{1F62F}', '\u{1F630}', '\u{1F631}', '\u{1F632}', '\u{1F633}', '\u{1F634}', '\u{1F635}', '\u{1F636}', '\u{1F637}', '\u{1F638}', '\u{1F639}', '\u{1F63A}', '\u{1F63B}', '\u{1F63C}', '\u{1F63D}', '\u{1
'\u{1F3E4}', '\u{1F3E5}', '\u{1F3E6}', '\u{1F3E7}', '\u{1F3E8}', '\u{1F3E9}', '\u{1F3EA}', '\u{1F3EB}', '\u{1F3EC}', '\u{1F3ED}', '\u{1F3EE}', '\u{1F3EF}', '\u{1F3F0}', '\u{1F3F1}', '\u{1F3F2}', '\u{1F3F3}', '\u{1F3F4}', '\u{1F3F5}', '\u{1F3F6}', '\u{1F3F7}', '\u{1F3F8}', '\u26BD', '\u26BE', '\u26F8\uFE0F', '\u26F7\uFE0F', '\u{1F6F9}', '\u{1F6FC}', '\u{1F579}', '\u265F', '\u{1F9E9}'],
            'TravelPlaces': ['\u{1F680}', '\u{1F681}', '\u{1F682}', '\u{1F683}', '\u{1F684}', '\u{1F685}', '\u{1F686}', '\u{1F687}', '\u{1F688}', '\u{1F689}', '\u{1F68A}', '\u{1F68B}', '\u{1F68C}', '\u{1F68D}', '\u{1F68E}', '\u{1F68F}', '\u{1F690}', '\u{1F691}', '\u{1F692}', '\u{1F693}', '\u{1F694}', '\u{1F695}', '\u{1F696}', '\u{1F697}', '\u{1F698}', '\u{1F699}', '\u{1F69A}', '\u{1F69B}', '\u{1F69C}', '\u{1F69D}', '\u{1F69E}', '\u{1F69F}', '\u{1F6A0}', '\u{1F6A1}', '\u{1F6A2}', '\u{1F6A3}', '\u{1F6A4}', '\u{1F6A5}', '\u{1F6A6}', '\u{1F6A7}', '\u{1F6A8}', '\u{1F6A9}', '\u{1F6AA}', '\u{1F6AB}', '\u{1F6AC}', '\u{1F6AD}', '\u{1F6AE}', '\u{1F6AF}', '\u{1F6B0}', '\u{1F6B1}', '\u{1F6B2}', '\u{1F6B3}', '\u{1F6B4}', '\u{1F6B5}', '\u{1F6B6}', '\u{1F6B7}', '\u{1F6B8}', '\u{1F6B9}', '\u{1F6BA}', '\u{1F6BB}', '\u{1F6BC}', '\u{1F6BD}', '\u{1F6BE}', '\u{1F6BF}', '\u{1F6C0}', '\u{1F6C1}', '\u{1F6C2}', '\u{1F6C3}', '\u{1F6C4}', '\u{1F6C5}', '\u{1F6CB}', '\u{1F6CC}', '\u{1F6CD}', '\u{1F6CE}', '\u{1F6CF}', '\u{1F6D0}', '\u{1F6D1}', '\u{1F6D2}', '\u{1F6D3}', '\u{1F6D4}', '\u{1F6D5}', '\u{1F6D6}', '\u{1F6D7}', '\u{1F6D8}', '\u{1F6D9}', '\u{1F6DA}', '\u{1F6DB}', '\u{1F6DC}', '\u{1F6DD}', '\u{1F6DE}', '\u{1F6DF}', '\u{1F6E0}', '\u{1F6E1}', '\u{1F6E2}', '\u{1F6E3}', '\u{1F6E4}', '\u{1F6E5}', '\u{1F6F0}', '\u{1F6F1}', '\u{1F6F2}', '\u{1F6F3}', '\u{1F6F4}', '\u{1F6F5}', '\u{1F6F6}', '\u{1F6F7}', '\u{1F6F8}', '\u{1F6F9}', '\u{1F6FA}', '\u{1F6FB}', '\u{1F6FC}', '\u{1F6FD}', '\u{1F6FE}', '\u{1F6FF}', '\u{1F3D4}
            'Flags': ['\u{1F3F3}\uFE0F', '\u{1F3F4}', '\u{1F3C1}', '\u{1F6A9}', '\u{1F3F3}\uFE0F\u200D\u{1F308}', '\u{1F3F3}\uFE0F\u200D\u26A7\uFE0F', '\u{1F1FA}\u{1F1F8}', '\u{1F1E8}\u{1F1E6}', '\u{1F1F2}\u{1F1FD}', '\u{1F1EC}\u{1F1E7}', '\u{1F1EE}\u{1F1EA}', '\u{1F1EB}\u{1F1F7}', '\u{1F1E9}\u{1F1EA}', '\u{1F1EA}\u{1F1F8}', '\u{1F1EE}\u{1F1F9}', '\u{1F1F5}\u{1F1F9}', '\u{1F1F3}\u{1F1F1}', '\u{1F1E7}\u{1F1EA}', '\u{1F1E8}\u{1F1ED}', '\u{1F1E6}\u{1F1F9}', '\u{1F1F8}\u{1F1EA}', '\u{1F1F3}\u{1F1F4}', '\u{1F1E9}\u{1F1F0}', '\u{1F1EB}\u{1F1EE}', '\u{1F1F5}\u{1F1F1}', '\u{1F1E8}\u{1F1FF}', '\u{1F1EC}\u{1F1F7}', '\u{1F1F7}\u{1F1F4}', '\u{1F1FA}\u{1F1E6}', '\u{1F1EE}\u{1F1F3}', '\u{1F1F5}\u{1F1F0}', '\u{1F1E7}\u{1F1E9}', '\u{1F1EF}\u{1F1F5}', '\u{1F1F0}\u{1F1F7}', '\u{1F1E8}\u{1F1F3}', '\u{1F1F9}\u{1F1FC}', '\u{1F1ED}\u{1F1F0}', '\u{1F1F8}\u{1F1EC}', '\u{1F1F9}\u{1F1ED}', '\u{1F1FB}\u{1F1F3}', '\u{1F1E6}\u{1F1FA}', '\u{1F1F3}\u{1F1FF}', '\u{1F1FF}\u{1F1E6}', '\u{1F1E7}\u{1F1F7}', '\u{1F1E6}\u{1F1F7}', '\u{1F1E8}\u{1F1F1}', '\u{1F1E8}\u{1F1F4}', '\u{1F1F5}\u{1F1EA}', '\u{1F1EA}\u{1F1EC}', '\u{1F1F9}\u{1F1F7}', '\u{1F1EE}\u{1F1F1}', '\u{1F1F8}\u{1F1E6}', '\u{1F1E6}\u{1F1EA}']
        };
        ...
        function searchEmojis(query) {
            emojiSearchQuery = String(query || '').trim().toLowerCase();
            renderEmojiGrid();
        }
        ...
        function renderEmojiGrid() {
            ...
            if (emojiSearchQuery) {
                const qRaw = String(emojiSearchQuery || '').trim().toLowerCase();
                const qCompact = normalizeEmojiSearchToken(qRaw);
                ...
                if (!qRaw) {
                    ...
                    const searchTerms = {
                        'heart': ['\u2764\uFE0F', '\u{1F9E1}', '\u{1F49B}', '\u{1F49A}', '\u{1F499}', '\u{1F49C}', '\u{1F5A4}', '\u{1F90D}', '\u{1F90E}', '\u{1F495}', '\u{1F49E}', '\u{1F493}', '\u{1F497}', '\u{1F496}'],
                        'love': ['\u2764\uFE0F', '\u{1F49B}', '\u{1F49C}', '\u{1F495}', '\u{1F496}', '\u{1F497}', '\u{1F498}', '\u{1F49D}', '\u{1F618}', '\u{1F970}'],
                        'star': ['\u2B50', '\u{1F31F}', '\u2728', '\u{1F4AB}', '\u{1F320}'],
                        'fire': ['\u{1F525}', '\u{1F4A5}', '\u26A1', '\u2604'],
                        'check': ['\u2705', '\u2611\uFE0F', '\u2714\uFE0F', '\u{1F7E2}'],
                        'warning': ['\u26A0', '\u{1F6A8}', '\u2757', '\u203C', '\u{1F514}'],
                        'book': ['\u{1F4DA}', '\u{1F4D6}', '\u{1F4D5}', '\u{1F4D7}', '\u{1F4D8}', '\u{1F4D9}', '\u{1F4DD}'],
                        'write': ['\u{1F4DD}', '\u270F\uFE0F', '\u{1F58A}\uFE0F', '\u{1F58B}\uFE0F', '\u{1F58C}\uFE0F'],
                        'folder': ['\u{1F4C1}', '\u{1F4C2}', '\u{1F5C2}\uFE0F', '\u{1F5C3}\uFE0F', '\u{1F4E6}'],
                        'work': ['\u{1F4BC}', '\u{1F3E2}', '\u{1F4BB}', '\u{1F4CA}', '\u{1F4C8}', '\u{1F9E0}', '\u{1F9FE}'],
                        'school': ['\u{1F3EB}', '\u{1F393}', '\u{1F4DA}', '\u{1F9EE}', '\u{1F4DD}', '\u{1F4C5}'],
                        'home': ['\u{1F3E0}', '\u{1F3E1}', '\u{1F6CB}', '\u{1F6CF}', '\u{1FAB4}'],
                        'idea': ['\u{1F4A1}', '\u{1F9E0}', '\u{1F4AD}', '\u{1F914}'],
                        'time': ['\u23F0', '\u231A', '\u23F1', '\u23F3', '\u{1F553}', '\u{1F4C5}', '\u{1F5D3}'],
                        'watch': ['\u231A', '\u23F0', '\u23F1', '\u23F2', '\u{1F570}', '\u{1F550}', '\u{1F551}', '\u{1F552}', '\u{1F553}', '\u{1F554}', '\u{1F555}', '\u{1F556}', '\u{1F557}', '\u{1F558}', '\u{1F559}', '\u{1F55A}', '\u{1F55B}'],
                        'calendar': ['\u{1F4C5}', '\u{1F5D3}', '\u{1F4C6}', '\u23F0'],
                        'clock': ['\u23F0', '\u231A', '\u{1F550}', '\u{1F551}', '\u{1F552}', '\u{1F553}'],
                        'music': ['\u{1F3B5}', '\u{1F3B6}', '\u{1F3A7}', '\u{1F3A4}', '\u{1F3B9}', '\u{1F3B8}', '\u{1F941}'],
                        'art': ['\u{1F3A8}', '\u{1F58C}\uFE0F', '\u{1F58D}', '\u{1F5BC}\uFE0F', '\u{1F3AD}', '\u{1F3AC}'],
                        'photo': ['\u{1F4F7}', '\u{1F4F8}', '\u{1F3A5}', '\u{1F4F9}', '\u{1F39E}\uFE0F'],
                        'video': ['\u{1F3A5}', '\u{1F4F9}', '\u{1F3AC}', '\u{1F4FA}'],
                        'tech': ['\u{1F4BB}', '\u{1F4F1}', '\u2328\uFE0F', '\u{1F5A5}\uFE0F', '\u{1F9EE}', '\u{1F50B}', '\u{1F50C}'],
                        'game': ['\u{1F3AE}', '\u{1F579}', '\u{1F3B2}', '\u265F', '\u{1F9E9}', '\u{1F3AF}'],
                        'sport': ['\u26BD', '\u{1F3C0}', '\u{1F3C8}', '\u26BE', '\u{1F3BE}', '\u{1F3C3}', '\u{1F6B4}', '\u{1F3CB}'],
                        'run': ['\u{1F3C3}', '\u{1F3C3}\u200D\u2640\uFE0F', '\u{1F45F}', '\u{1F4A8}'],
                        'travel': ['\u2708\uFE0F', '\u{1F697}', '\u{1F680}', '\u{1F681}', '\u26F5', '\u{1F9F3}', '\u{1F5FA}'],
                        'car': ['\u{1F697}', '\u{1F695}', '\u{1F699}', '\u{1F3CE}', '\u{1F6FB}'],
                        'train': ['\u{1F686}', '\u{1F68A}', '\u{1F689}', '\u{1F682}'],
                        'plane': ['\u2708\uFE0F', '\u{1F6EB}', '\u{1F6EC}', '\u{1F6E9}'],
                        'money': ['\u{1F4B5}', '\u{1F4B4}', '\u{1F4B6}', '\u{1F4B7}', '\u{1F4B0}', '\u{1FA99}', '\u{1F4B8}', '\u{1F4B3}'],
                        'shopping': ['\u{1F6D2}', '\u{1F6CD}\uFE0F', '\u{1F3F7}', '\u{1F4E6}', '\u{1F4B3}'],
                        'party': ['\u{1F389}', '\u{1F38A}', '\u{1F388}', '\u{1F381}', '\u{1F380}', '\u{1F37E}', '\u{1F382}'],
                        'smile': ['\u{1F600}', '\u{1F603}', '\u{1F604}', '\u{1F601}', '\u{1F60A}', '\u{1F642}', '\u{1F60E}'],
                        'sad': ['\u{1F622}', '\u{1F62D}', '\u{1F61E}', '\u{1F614}', '\u{1F641}'],
                        'laugh': ['\u{1F602}', '\u{1F923}', '\u{1F606}', '\u{1F639}'],
                        'hand': ['\u{1F44D}', '\u{1F44E}', '\u{1F44F}', '\u{1F64C}', '\u{1F64F}', '\u{1F91D}', '\u{1F44B}'],
                        'people': ['\u{1F9D1}', '\u{1F468}', '\u{1F469}', '\u{1F465}', '\u{1FAC2}', '\u{1F9D8}'],
                        'dog': ['\u{1F436}', '\u{1F415}', '\u{1F9AE}'],
                        'cat': ['\u{1F431}', '\u{1F408}', '\u{1F63A}'],
                        'animal': ['\u{1F436}', '\u{1F431}', '\u{1F42D}', '\u{1F430}', '\u{1F98A}', '\u{1F43B}', '\u{1F43C}', '\u{1F42F}', '\u{1F981}'],
                        'bird': ['\u{1F426}', '\u{1F985}', '\u{1F989}', '\u{1F427}', '\u{1F986}'],
                        'fish': ['\u{1F41F}', '\u{1F420}', '\u{1F42C}', '\u{1F988}'],
                        'plant': ['\u{1F331}', '\u{1F33F}', '\u{1F340}', '\u{1F332}', '\u{1F338}', '\u{1F339}', '\u{1F337}', '\u{1FAB4}'],
                        'weather': ['\u2600', '\u{1F324}', '\u26C5', '\u2601', '\u{1F327}', '\u26C8', '\u2744', '\u{1F308}'],
                        'sun': ['\u2600', '\u{1F31E}', '\u{1F324}'],
                        'moon': ['\u{1F319}', '\u{1F31A}', '\u{1F31B}', '\u{1F31C}'],
                        'rocket': ['\u{1F680}', '\u{1F6F8}', '\u{1F6F0}'],
                        'lock': ['\u{1F512}', '\u{1F513}', '\u{1F510}', '\u{1F511}'],
                        'tool': ['\u{1F6E0}', '\u{1F527}', '\u{1F528}', '\u{1FA9B}', '\u2699', '\u{1F9F0}'],
                        'food': ['\u{1F34E}', '\u{1F355}', '\u{1F354}', '\u{1F32E}', '\u{1F35C}', '\u{1F363}', '\u{1F370}'],
                        'fruit': ['\u{1F34E}', '\u{1F34C}', '\u{1F353}', '\u{1F347}', '\u{1F34D}', '\u{1F96D}'],
                        'coffee': ['\u2615', '\u{1F375}', '\u{1F9CB}'],
                        'health': ['\u{1F4AA}', '\u{1FA7A}', '\u{1F48A}', '\u{1F489}', '\u{1F9D8}'],
                        'sleep': ['\u{1F634}', '\u{1F6CC}', '\u{1F4A4}'],
                        'flag': ['\u{1F3F3}\uFE0F', '\u{1F3F4}', '\u{1F3C1}', '\u{1F6A9}', '\u{1F1FA}\u{1F1F8}', '\u{1F1EC}\u{1F1E7}', '\u{1F1EF}\u{1F1F5}', '\u{1F1EE}\u{1F1F3}']
                    };
                    ...
                    const matchedByCategory = Object.entries(emojiCategories)
                        .filter(([category]) => {
                            ...
                            const spacedCategory = rawCategory.replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase();
                            const compactCategory = normalizeEmojiSearchToken(spacedCategory);
                            const compactMatch = qCompact
                                ? (compactCategory.includes(qCompact) || qCompact.includes(compactCategory))
                                : false;
                            return spacedCategory.includes(qRaw)
                                || compactMatch;
                        })
                        .flatMap(([, list]) => list);
                    ...
                    const matchedByAlias = Object.entries(searchTerms)
                        .filter(([term]) => {
                            ...
                            const compactTerm = normalizeEmojiSearchToken(rawTerm);
                            const compactMatch = qCompact
                                ? (compactTerm.includes(qCompact) || qCompact.includes(compactTerm))
                                : false;
                            return rawTerm.includes(qRaw)
                                || qRaw.includes(rawTerm)
                                || compactMatch;
                        })
                        .flatMap(([, list]) => list);

                    // Allow searching by directly typing/pasting an emoji glyph.
                    const matchedByGlyph = allEmojis.filter(emoji => emoji.includes(qRaw));

                    // Match generated per-emoji keyword map (loaded from emoji-keywords.generated.js).
                    const keywordMap = (typeof window !== 'undefined'
                        && window.emojiSearchKeywordMap
                        && typeof window.emojiSearchKeywordMap === 'object')
                        ? window.emojiSearchKeywordMap
                        : null;
                    const matchedByKeyword = keywordMap
                        ? allEmojis.filter(emoji => {
                            const tokens = keywordMap[emoji];
                            if (!Array.isArray(tokens) || tokens.length === 0) return false;
                            return tokens.some(token => {
                                const rawToken = String(token || '').toLowerCase();
                                if (!rawToken) return false;
                                const tokenCompact = normalizeEmojiSearchToken(rawToken);
                                const compactMatch = qCompact
                                    ? (tokenCompact.includes(qCompact) || qCompact.includes(tokenCompact))
                                    : false;
                                return rawToken.includes(qRaw)
                                    || qRaw.includes(rawToken)
                                    || compactMatch;
                            });
                        })
                        : [];
                    ...
                    const filteredInCategory = emojis.filter(emoji => candidateSet.has(emoji));
                    if (filteredInCategory.length > 0 || currentEmojiCategory === 'All') {
                        emojis = filteredInCategory;
                    } else {
                        emojis = allEmojis.filter(emoji => candidateSet.has(emoji));
                    }
                }
            }
