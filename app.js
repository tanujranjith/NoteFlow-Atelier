...
// Google Drive API Configuration
    ...
    const MAX_COMMITS_PER_DAY = Infinity;
        ...
        // Quick Task (Sidebar) Functions
        let todoCategories = new Set();

        function getQuickTasks() {
            return tasks.filter(task => task.origin === 'quick');
        }

        function loadTodos() {
            todoCategories = new Set();
            getQuickTasks().forEach(task => {
                if (task.category && task.category !== 'none') todoCategories.add(task.category);
            });
        ...
        function addTodo() {
            ...
            const scheduleType = recurring === 'daily' ? 'daily' : recurring === 'weekly' ? 'weekly' : 'once';
            const weeklyDays = scheduleType === 'weekly'
                ? [new Date(dueDate || Date.now()).getDay()]
                : [];
        ...
        function updateTodoProgress() {
            ...
            const completedToday = (dayStates[todayKey] && dayStates[todayKey].completedTaskIds) || [];
            const completed = quickTasks.filter(task => completedToday.includes(task.id) || (task.scheduleType === 'once' && task.isActive === false)).length;
        ...
        function getFilteredTodos() {
            ...
            return getQuickTasks().filter(todo => {
                const completed = todo.scheduleType === 'once' ? todo.isActive === false : false;
        ...
        function renderTodos() {
            ...
            todoList.innerHTML = filteredTodos.map(todo => {
                const completedToday = (dayStates[today()] && dayStates[today()].completedTaskIds) || [];
                const isCompleted = completedToday.includes(todo.id) || (todo.scheduleType === 'once' && todo.isActive === false);
                ...
                const metaItems = [];
                if (todo.dueDate) {
                    const overdueClass = !isCompleted && isOverdue(todo.dueDate) ? 'overdue' : '';
                ...
                if (todo.scheduleType && todo.scheduleType !== 'once') {
                    metaItems.push(`<span class="todo-meta-item"><i class="fas fa-redo"></i> ${todo.scheduleType}</span>`);
                }
                ...
                return `
                    <div class="todo-item">
                        ...
                        <span class="todo-text ${isCompleted ? 'completed' : ''}">${escapeHtml(todo.title)}${priorityBadge}</span>
        ...
        function syncSettingsControls() {
            ...
            themeButtons.forEach(btn => {
                const isDark = appSettings && appSettings.theme === 'dark';
        ...
        function initWorkspaceUI() {
            ...
            if (quickAddNoteBtn) quickAddNoteBtn.addEventListener('click', () => createNewPage());
