...
A friendly, hands-on walkthrough of your **first session** with Sutra — from opening the app to saving your first backup. No accounts, no setup files, no cloud. Just you and one private workspace.
...
> Everything here is local. Nothing leaves your browser. Your completed setup is included in your `.sutra` backups.
...
> The fastest way around the whole app is the **Command Palette**: press **Ctrl/⌘+K** (anywhere outside an editor) and start typing.
...
> On the AP Study view, `Ctrl/⌘+K` is reserved for **Add subject** rather than the Command Palette.
...
1. Open **Review** (top tab, or `Ctrl/⌘+K → Start review session`).
...
This is the single most important habit. There is no Sutra cloud — your backup *is* your safety net.
...
- Old **`.atelier`** backups still import — nothing is broken.
- **API keys are never in the backup.** Re-enter them after restoring on a new device.
- `.sutra` files are **not encrypted** — store them like any personal file.
